...
def findReason(t, xml, id2reason, outer_control=None):
    ...
    for child in xml:
        ...
        if child.tag != "action" and child.tag!= "ref_event":
            ...
            try:
                ...
                for sub_child in child:
                    if sub_child.tag == "arg":
                        trigger_text = str(sub_child.text).lower()
                        if "other than" in trigger_text and child.get("type") == "receive":
                            other_than_receive = True

                        elif "other than" in trigger_text and child.get("type") == "send":
                            other_than_send = True
                        ...
                        for sub_sub_child in sub_child.iter("ref_event"):
                            ...
                            if reason + argument not in events:
                                events.append(reason + argument)
                        break
                    if sub_child.tag == "ref_event":
                        reason = id2reason[sub_child.get("id")]
                        if reason + argument not in events:
                            events.append(reason + argument)
                        break
                if child.text != None :
                ...
                pass
        elif child.tag == "ref_event":
            if child.get("type") == 'None':
                continue
            argument = omap[child.get("type")]
            reason = id2reason[child.get("id")]
            if reason + argument not in events:
                events.append(reason + argument)
