...
class BERT_BERT_MLP(nn.Module):
    ...
    def _get_bert_features(self, x, x_feats, x_len, x_chunk_len):
        ...
        tensor_seq = torch.zeros((len(x_len), max_seq_len, 768)).float().device()
    ...
    def _get_chunk_features(self, feats, templates):
        ...
        preds = torch.zeros((feats.shape[1],self.tokenizer.vocab_size)).device()

        for template in templates:
            pred = torch.zeros((0,self.tokenizer.vocab_size)).device()
    ...
    def neg_log_likelihood(self, x, x_feats, x_len, x_chunk_len, y):
        ...
        if self.is_soft:
            output = self._get_chunk_features_soft(feats, self.templates.device())
        else:
        ...
        loss = self.crossEntropyLoss(output, tag_word.device())
...
def evaluate_bert(model, test_dataloader):
    ...
    for x, x_feats, x_len, x_chunk_len, y in test_dataloader:
        x = x.device()
        x_feats = x_feats.device()
        x_len = x_len.device()
        x_chunk_len = x_chunk_len
        y = y
...
def main():
    ...
    args.savedir_fold = "output/checkpoint_TCP.pt"
