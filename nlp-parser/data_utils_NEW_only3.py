...
def add_label_token_bert(model, tokenizer, procotol, label_map_path, domain_label_map_path, tag2index, redundancy=False):
    ...
    if domain_label_map_path:
        domain_label_map = json.load(open(domain_label_map_path, 'r'))
        for k, v in domain_label_map.items():
            label_map[k] += v
            if not redundancy:
    ...
    # print(label_map)
    # for only 3 tags
    tmp_label_map = label_map
    label_map = {}
    for k, v in tmp_label_map.items():
        if k[2:] not in ['TRIGGER', 'ACTION', 'TRANSITION', 'O']:
            label_map['B-O'] += v
        else:
            label_map[k] = v
    ...
    for token in sorted_add_tokens:
        ...
        if len(index)>1:
            ...
            index = index[0]
        tag2index[token if token[2:]!='O' else 'O'] = index
        # assert index>=num_tokens, (index, num_tokens, token)
        if isinstance(label_map[token], list):
            indexes = tokenizer.convert_tokens_to_ids(label_map[token])
        else:
            indexes = tokenizer.convert_tokens_to_ids([label_map[token]])
        indexes = [id for id in indexes if id != 100] # remove ids that not in vocabulary. id=100 => [UNK]
...
def bert_sequences_tbased(X, y, max_chunks, max_tokens, id2word, bert_model):
...
def bert_sequences(X, y, max_chunks, max_tokens, id2word, tokenizer, tag2index, id2tag, is_y_with_tag_index=True):
    ...
    for x, y_ in zip(X, y):
        ...
        if is_y_with_tag_index:
            ...
            # for only 3 tags
            y_ = [tag2index[id2tag[i] if id2tag[i][2:] in ['TRIGGER', 'ACTION', 'TRANSITION', 'O'] else 'O'] for i in y_]
...
def srl_events(predictor, action_str, current_tag, def_states, protocol):
    tag_type = None; tags = []
    if not re.match('.*(syn-sent|syn-received).*', action_str): ## syn-sent | syn-received is states of TCP
...
def write_results(X_test_data, y_test_trans, y_pred_trans, level_h_trans, level_d_trans, id2word, def_states, def_events, def_events_constrained, protocol, cuda_device):
    ...
    for x, y_g, y_p, h, d in zip(X_test_data, y_test_trans, y_pred_trans, level_h_trans, level_d_trans):
        ...
        while (i < len(x_trans)):
            ...
            word = x_trans[i]
            pred_tag = y_p[i]
            # closed = True ## todo : for some bug
            current_tag = pred_tag
            if prev_tag is not None and not same_span(current_tag, prev_tag) and prev_tag != 'O': # 如果当前tag和之前的tag不同且前一个tag不是空或"O"，则加上闭标签
            ...
            # if current_tag is None or (not same_span(current_tag, prev_tag) and current_tag in testset):
                ...
                if tag_type is None or current_tag[2:].lower() != "action": # 如果没有type或者不是action，加上tag的开标签
                    offset_str = "".join(['\t'] * offset)
                    ret_str += "\n" + offset_str + "<{}>".format(current_tag[2:].lower())
                    cur_str += "\n" + offset_str + "<{}>".format(current_tag[2:].lower())
                else: # 如果是aciton且有type，则开标签中加入type
                    offset_str = "".join(['\t'] * offset)
