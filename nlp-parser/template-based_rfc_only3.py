...
import data_utils_NEW_only3 as data_utils
import features
...
class BERT_BERT_MLP(nn.Module):
    ...
    def __init__(self, bert_model_name, chunk_hidden_dim, max_chunk_len, max_seq_len,
                 feat_sz, batch_size, output_dim, use_features=False, bert_freeze=0, templates=None, is_soft=False, template_len=10):
        ...
        self.templates = templates if not is_soft else nn.Parameter(nn.init.normal_(torch.empty(1,template_len,768, requires_grad=True)))
    ...
    def _get_chunk_features(self, feats, templates):
        ...
        # chunk_feat = torch.zeros((0,self.tokenizer.vocab_size)).cuda()
        preds = torch.zeros((feats.shape[1],self.tokenizer.vocab_size)).cuda()
        ...
        for template in templates:
            ...
            #### window ####
            # window_size = 2k + 1 : k + 1 + k
            # eg: k = 16, window_size = 33 : 16 + 1 + 16
            # print('with windows')
            k = 8
            l = feats.shape[1]
            for i in range(l):
                left = 0 if i < k else i-k
                right = i+k if i+k < l-1 else l-1
    ...
    def _get_chunk_features_soft(self, feats, templates):
        ...
        for i in range(l):
            ...
            pred = torch.cat((pred, x[:,-len:,:].mean(dim=1)), 0)
        return pred
    ...
    def tag2word(self, tag):
        ...
        # for only3
        labelwords = ['trigger', 'action', 'other', 'transition']
    ...
    def neg_log_likelihood(self, x, x_feats, x_len, x_chunk_len, y):
        ...
        if self.is_soft:
            output = self._get_chunk_features_soft(feats, self.templates.cuda())
        else:
...
def evaluate_bert(model, test_dataloader):
    ...
    for x, x_feats, x_len, x_chunk_len, y in test_dataloader:
        ...
        if model.is_soft:
            output = model._get_chunk_features_soft(feats, model.templates.cuda())
        else:
            output = model._get_chunk_features(feats, model.templates)
...
def main():
    ...
    parser.add_argument('--multi_template_num', type=int, default=1)
    parser.add_argument('--template_id', type=int, default=0)
    parser.add_argument('--soft_template', default=False, action='store_true')
    parser.add_argument('--soft_template_len', type=int, default=10)
    ...
    if args.soft_template:
        args.savedir_fold = os.path.join(args.savedir, "output/checkpoint_soft_{}_{}.pt".format(
            args.soft_template_len,
            args.protocol))
        args.log_file = 'training_statics_soft_{}_{}.txt'.format(
            args.protocol,
            args.soft_template_len)
    else:
        args.savedir_fold = os.path.join(args.savedir, "output/checkpoint_{}_{}_{}.pt".format(
            'multi' if args.multi_template else 'single',
            args.multi_template_num if args.multi_template else args.template_id,
            args.protocol))
        args.log_file = 'training_statics_{}_{}_{}.txt'.format(
            args.protocol,
            'multi' if args.multi_template else 'single',
            args.multi_template_num if args.multi_template else args.template_id)
    ...
    # for only3
    X_train_data_orig, y_train, level_h, level_d = data_utils.get_data_tb_only3(args.train, word2id, tag2id, pos2id, id2cap, stem2id, id2word, transition_counts, partition_sentence=args.partition_sentence)
    X_test_data_orig, y_test, level_h, level_d = data_utils.get_data_tb_only3(args.test, word2id, tag2id, pos2id, id2cap, stem2id, id2word, partition_sentence=args.partition_sentence)
    ...
    vocab_size = len(stem2id) # 1287
    pos_size = len(pos2id) # 40
    ...
    X_train_data, y_train, x_len, x_chunk_len = \
        data_utils.bert_sequences_tbased(X_train_data_orig, y_train, max_chunks, max_tokens, id2word, args.bert_model)
    X_test_data, y_test, x_len_test, x_chunk_len_test = \
        data_utils.bert_sequences_tbased(X_test_data_orig, y_test, max_chunks, max_tokens, id2word, args.bert_model)
    ...
    if args.soft_template:
        ...
        if args.multi_template:
            templates = templates_list[:args.multi_template_num]
        else:
    ...
    model = BERT_BERT_MLP(args.bert_model,
                          ...
                          use_features=args.features, bert_freeze=10, templates=templates, is_soft=args.soft_template, template_len=args.soft_template_len)
    model.cuda()
