...
class BERT_BERT_MLP(nn.Module):
    ...
    def forward_crf(self, x, x_feats, x_len, x_chunk_len, y, device, index2id):
...
def evaluate_bert(model, test_dataloader, device, index2id):
    ...
    for x, x_feats, x_len, x_chunk_len, y in test_dataloader:
        ...
        model.zero_grad()
        # Get loss
        loss, logits = model.forward_crf(x, x_feats, x_len, x_chunk_len, y, device, index2id)
        #print(loss.item())
        total_loss_dev += loss.item()
        labels_ids = [x[0] for x in sorted(index2id.items(), key=lambda x: x[1])]

        batch_p = []
        for x_len_i, sequence in zip(x_len, logits[:,:,labels_ids]):
            #print("sequence.shape", sequence[:x_len_i].shape)
            # Find the best path, given the features.
            # set emission score of START_TAG and END_TAG to zero
            feats = torch.cat((sequence[:x_len_i], torch.zeros(sequence.shape[0],2).to(device)), -1)
            score, tag_seq = model._viterbi_decode(feats)
            batch_p.append(tag_seq)
...
def main():
    ...
    args.savedir_fold = os.path.join(args.savedir, "output/checkpoint_only3_TF{}{}{}{}{}_{}.pt".format(
        "_domain+datasearch" if args.domain_label_map_path else "_datasearch",
        f"_lr{args.learning_rate}",
        f"_p{args.patience}",
        "_warmup" if args.warmup else "",
        "_crf" if args.crf else "",
        args.protocol))
    ...
    X_train_data, y_train, x_len, x_chunk_len = \
        data_utils.bert_sequences(X_train_data_orig, y_train, max_chunks, max_tokens, id2word, tokenizer, tag2index, id2tag, False if args.crf else True)
    X_test_data, y_test, x_len_test, x_chunk_len_test = \
        data_utils.bert_sequences(X_test_data_orig, y_test, max_chunks, max_tokens, id2word, tokenizer, tag2index, id2tag, False if args.crf else True)
    ...
    if args.do_eval:
        ...
        if args.write_results:
            output_xml = os.path.join(args.outdir, "{}{}_only3.xml".format(args.protocol, "-R" if args.heuristics else ""))
